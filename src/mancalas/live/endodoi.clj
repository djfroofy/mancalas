(ns mancalas.live.endodoi
    (:use [mancalas.endodoi :only
...
(defn play [game-atom row-index]
  (let [player (:turn @game-atom)
    ...
    (if row-index
      (if (or
           (not (valid-move? board player row-index))
           (:end? @game-atom))
        (println "Invalid move")
        (do
          (when-player-b
            (println (str player " to move. row-index: " row-index ", game-state:\n\t" @game-atom)))
          (swap! game-atom advance-game row-index)
          (pprint-board (:board @game-atom))
          (if-player-a
            (let [moves (valid-moves (:board @game-atom) :player-b)
                  move (if (not-empty moves) (rand-nth moves) nil)]
              (recur game-atom move))
            (do
